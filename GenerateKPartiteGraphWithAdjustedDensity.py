...
def generate_k_colorable_graph(*n_sets, p_density=0.8):

    G = nx.Graph()
    sets = []
    color = 1
    start_node_id = 0  
    total_nodes = sum(n_sets)
    ...
    forbidden_edges = sum(n * (n - 1) // 2 for n in n_sets)
...
p_density = 0.8 
G = generate_k_colorable_graph(*n_sets, p_density=p_density)
