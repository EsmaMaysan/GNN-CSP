...
def generate_queen_graph(size=8):
    G = nx.Graph()
                ...
                for dj in range(-size + 1, size):
                    if abs(di) == abs(dj) and di != 0 and dj != 0:
                        ...
                        if 0 <= ni < size and 0 <= nj < size:
                            G.add_edge(node, ni * size + nj)
    
    return G

def draw_queen_graph_circular(G):
    pos = {}
    ...
    plt.title("Queen Graph 6x6 (Circular Layout)")
    plt.axis("off")
    plt.show()
...
def train_and_evaluate():
    ...
    # compare with known chrmtc nmbr
    known_chromatic_number = 9 
